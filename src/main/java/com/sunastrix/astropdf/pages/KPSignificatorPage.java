...
public class KPSignificatorPage extends BasePage {
	...
	public KPSignificatorPage(DesktopHoroNew desktopHoro) {
	...
	public PageInfo drawPage(PDDocument document, PDType0Font poppinsRegularFont, PDType0Font krutiDevRegularFont) {
		...
		try (PDPageContentStream cs = new PDPageContentStream(document, pageDetail.getPage())) {
			...
			float tableHeight = 26 + 12 * 20 + 5 * 1;
			...
			drawBgWithHeader(bgX, bgY, bgWidth, bgHeight, headWidth, headHeight, headMargin, "Hkko funsZ'ku", 14);
			drawBhavSignificatorTable(tableX, tableY, tableWidth);
			tableX = 39f;
			tableY = 320f;
			tableWidth = 514f;
			...
			headHeight = 26f;
			headMargin = 13.2f;
			drawBgWithHeader(bgX, bgY, bgWidth, bgHeight, headWidth, headHeight, headMargin, "xzg funsZ'ku¼ [kkdk 2½",
					14);
			drawPlanetSignificatorView2Table(tableX, tableY, tableWidth);
	...
	private void drawBhavSignificatorTable(float x, float y, float width) throws IOException {
		float tx = x;
		float ty = y;
		...
		populateBhavSigniFicatorTable(x, y, width);
	}

	void populateBhavSigniFicatorTable(float x, float y, float width) throws IOException {
		String[] heading = constantHindi.bhavSignificatorHeading;
		...
		for (int i = 0; i < list.size(); i++) {
			...
			tx = x;
			drawColorShape.drawText(tx + 20, starty, String.valueOf(houseSignificatorsBean.getHouseNo()),
					krutiDevRegularFont, 16, planetColors[i]);
			tx += columnWidth[0];
			drawColorShape.drawText(tx + 20, starty, houseSignificatorsBean.getSignificator(), krutiDevRegularFont, 16,
					planetColors[i]);
			starty -= rowHeight + 1;
		}
	}

	private void drawPlanetSignificatorView2Table(float x, float y, float width) throws IOException {
		float tx = x;
		float ty = y;
		float headerHeight = 26f;
		float rowHeight = 20f;
		...
		contentStream.stroke();
		contentStream.restoreGraphicsState();
		populatePlanetSignifiactionView2Table(x, y, width);
	}

	void populatePlanetSignifiactionView2Table(float x, float y, float width) throws IOException {
		String[] heading = constantHindi.planetSignificator2Heading;
		...
		for (int i = 0; i < list.size(); i++) {
			...
			tx = x;
			drawColorShape.drawText(tx + 20, starty, planetSignificationView2Bean.getPlaNo(), krutiDevRegularFont, 16,
					planetColors[i]);
			tx += firstColumnWidth;
			drawColorShape.drawText(tx + 20, starty, planetSignificationView2Bean.getL1(), poppinsRegularFont, 12,
					planetColors[i]);
			tx += columnWidth;
			drawColorShape.drawText(tx + 20, starty, planetSignificationView2Bean.getL2(), poppinsRegularFont, 12,
					planetColors[i]);
			tx += columnWidth;
			drawColorShape.drawText(tx + 20, starty, planetSignificationView2Bean.getL3(), poppinsRegularFont, 12,
					planetColors[i]);
			tx += columnWidth;
			drawColorShape.drawText(tx + 20, starty, planetSignificationView2Bean.getL4(), poppinsRegularFont, 12,
					planetColors[i]);
			starty -= rowHeight + 1;
