...
public class PlanetConsiderationPage extends BasePage {
	...
	public PlanetConsiderationPage(DesktopHoroNew desktopHoro, BirthDetailBean birthDetailBean) {
		...
		for (int i = 0; i < list.size(); i++) {
			arrayList.add(new ParagraphDetail(heading[i], list.get(i).getConsideration().replace("\n", " ")));
		}
	}

	public PageInfo drawPage(PDDocument document, PDType0Font poppinsRegularFont, PDType0Font krutiDevRegularFont) {
		String pageHeading = "xzg fopkj";
	...
	private float createNewPage(PDDocument document, PDType0Font poppinsRegularFont, PDType0Font krutiDevRegularFont)
		...
		drawCornerImages();
		drawHeader(pageWidth, pageHeight, krutiDevRegularFont, "xzg fopkj");
		drawFooter(poppinsRegularFont, poppinsRegularFont, 15);
