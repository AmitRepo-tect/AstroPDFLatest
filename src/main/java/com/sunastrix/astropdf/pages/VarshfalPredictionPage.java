...
public class VarshfalPredictionPage extends BasePage {
	...
	public VarshfalPredictionPage(DesktopHoroNew desktopHoro, BirthDetailBean birthDetailBean) {
		...
		int birthYear = Integer.parseInt(birthDetailBean.getDateTimeBean().getYear());
		int yearCount = currentYear - birthYear;
		munthaBhav = varshfalCalculation.getMunthaBhav(birthDetailBean, currentYear, yearCount);
		munthaDesc = varshfalCalculation.getMunthaDesc(munthaBhav - 1).getDesc();
		arrayList.add(new ParagraphDetail("eqaFkk Hkko% " + munthaBhav, munthaDesc));
		ArrayList<VarshfalDashaBean> list = varshfalCalculation.getBhavishyafal(birthDetailBean, currentYear,
				yearCount);
		for (int i = 0; i < list.size(); i++) {
			VarshfalDashaBean varshfalDashaBean = list.get(i);
			String heading = varshfalDashaBean.getStartTime() + " & " + varshfalDashaBean.getEndTime();
			arrayList.add(new ParagraphDetail(heading, varshfalDashaBean.getResult()));
		}
	}

	public PageInfo drawPage(PDDocument document, PDType0Font poppinsRegularFont, PDType0Font krutiDevRegularFont) {
		String pageHeading = "o\"kZQy Qykns'k";
	...
	private float createNewPage(PDDocument document, PDType0Font poppinsRegularFont, PDType0Font krutiDevRegularFont)
		...
		drawCornerImages();
		drawHeader(pageWidth, pageHeight, krutiDevRegularFont, "o\"kZQy Qykns'k ");
		drawFooter(poppinsRegularFont, poppinsRegularFont, 15);
	...
	/*
	 * private float printParagraph(float startX, float startY, String heading,
	 * List<String> lines, int colorIndex) throws Exception {
	 * 
	 * final float headingHeight = 25f; final float fontSize = 12f; final float
	 * headingFontSize = 16f; final float leading = 15f; final float availableWidth
	 * = pageWidth - startX - 20f; final float cornerRadius = 8f; final float
	 * verticalPadding = 6f; final float verticalLineWidth = 2.5f; float headerY =
	 * startY - headingHeight;
	 * 
	 * drawColorShape.drawGradientHeader(startX, headerY, availableWidth,
	 * headingHeight, cornerRadius, headerGradientColors[colorIndex][1],
	 * headerGradientColors[colorIndex][0], headerGradientColors[colorIndex][1],
	 * headerGradientColors[colorIndex][1]);
	 * 
	 * float headingY = headerY + 7f; drawShape.drawBoldText(startX + 15f, headingY,
	 * heading, (int) headingFontSize, krutiDevRegularFont, Color.WHITE); float
	 * firstLineY = headerY - 15f; float yPosition = firstLineY - 10f; for (String
	 * line : lines) { if (line == null || line.trim().isEmpty()) { continue; }
	 * drawColorShape.drawText(startX + 15f, yPosition, line, krutiDevRegularFont,
	 * fontSize, Color.BLACK); yPosition -= leading; } yPosition += leading; float
	 * bottomY = yPosition - 15f; float bodyTop = headerY; float bodyHeight =
	 * bodyTop - bottomY; if (bodyHeight > 0) { contentStream.saveGraphicsState();
	 * contentStream.setStrokingColor(headerGradientColors[colorIndex][1]);
	 * contentStream.setLineWidth(0.5f);
	 * drawColorShape.drawRoundedBottomRectangle(startX, bottomY, availableWidth,
	 * bodyHeight, cornerRadius); contentStream.stroke(); float lineX = startX + 6f;
	 * float lineTop = bodyTop - verticalPadding; float lineBottom = bottomY +
	 * verticalPadding; contentStream.setLineWidth(verticalLineWidth);
...
class ParagraphDetail {
	...
	ParagraphDetail(String heading, String content) {
