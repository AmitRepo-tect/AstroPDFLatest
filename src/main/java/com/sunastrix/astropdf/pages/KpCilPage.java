...
public class KpCilPage extends BasePage {
	...
	public KpCilPage(DesktopHoroNew desktopHoro) {
	...
	private void drawCILSubSubTable(float x, float y, float width) throws IOException {
		float tx = x;
		float ty = y;
	...
	void populateCILSubTable(float x, float y, float width) throws IOException {
		String[] heading = constantHindi.cusperInterLinksSubHeading;
		...
		firstColumnWidth = 60f;
		KPCilSubSubBean kpCilSubSubBean;
		for (int i = 0; i < list.size(); i++) {
			kpCilSubSubBean = list.get(i);
			tx = x;
			drawColorShape.drawText(tx + 10, starty, String.valueOf(kpCilSubSubBean.getPlanetName()),
					krutiDevRegularFont, 16, planetColors[i]);
			tx += firstColumnWidth;
			drawColorShape.drawText(tx + 10, starty, kpCilSubSubBean.getStar(), poppinsRegularFont, 12,
					planetColors[i]);
			tx += columnWidth;
			drawColorShape.drawText(tx + 10, starty, kpCilSubSubBean.getSub(), poppinsRegularFont, 12, planetColors[i]);
			tx += columnWidth;
			drawColorShape.drawText(tx + 10, starty, kpCilSubSubBean.getSubSub(), poppinsRegularFont, 12,
					planetColors[i]);
			tx += columnWidth;
			drawColorShape.drawText(tx + 10, starty, kpCilSubSubBean.getPosStatus(), poppinsRegularFont, 12,
					planetColors[i]);
